...
### Multivariate Exploration -------------------------------------------------------------  
 ...
 ## Multivariate Visual/Informal  
  ...
  has.view <- as.data.frame(lapply(sales.data[,12:20], 
  ...
  view.scores <- as.data.frame(lapply(sales.data[, 12:20], 
...
### Mapping ------------------------------------------------------------------------------  
  ...
  base.map + geom_path(data=beats.spf,
...
### Label outliers -----------------------------------------------------------------------
  ...
  ex.disc <- ggplot(x.data, aes(x=tot.sf, y=sale.price, 
                                color=as.factor(discordant))) +
    geom_point()+scale_x_log10()+scale_y_log10()
  
  mv.disc <- ggplot(x.data, aes(x=tot.sf, y=sale.price, color=as.factor(mah.col),
                     size=sqrt(mah.col)/2)) +
    geom_point()+scale_x_log10()+scale_y_log10()+
