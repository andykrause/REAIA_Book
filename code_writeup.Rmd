...
Next, the user must enter the directory where the data is stored, **data.dir** (the same directory from Chapter 4) and the directory where the code has been downloaded, **code.dir**. 
```{r eval=FALSE}
...
There a number of parameters that must be set to define the integration and cleaning process. They are as follows:

1. **sale.years**: The time range of sales to consider
2. **data.year**: The year from which to use the parcel and assessor data
3. **trans.limit**: The max number of transactions allowed per property in the time period (eliminates flips and other non-arms length situations)
4. **trim.list**: A list of fields and associated codes that are not considered to be valid sales transactions.  The details of these are explained more below.  
5. **over.write**: Whether or not to overwrite existing files in the database.  

Users are cautioned not to change these parameter unless they have appropriately downloaded different assessor or geographic files, and/or wish to change the scope of the sales considered.  
