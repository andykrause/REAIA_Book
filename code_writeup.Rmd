...
#### Integrate Assessors data

To better manage the various assessor's data files -- the sales, parcel and residential building information -- we combine them into a single SQLite database.  This also allows for more portability of the files.  This same database will contain the various iterations of the cleaned sales data as well.  The **convertCSVtoSQLite()** function is a custom function that we have developed for the task of combining various CSV files into a single SQLite database. If you don't want to see the progress of the process you can change the *verbose* parameter to FALSE. 
...
#### Initial Clean of the Sale Transactions
...
Code for making EDA Plots

## Chapter 9

Code for making outlier ID plots
